...
vcf file list:
    You require the list of vcf files to run this. Create with:
        find /Sample_Data/ -type f -name "*variants-annotated.vcf" > vcf_file_list.txt

Useage: Run in command line
    
    With coordinates for f.e. estrogen receptor:
        python ../gitrepo/vcf_location_table.py -s vcf_file_list.txt -o location_table_ESR1.tsv -n ESR1 -r ../gene_ranges.bed
...
parser.add_argument('--gene', '-n', required=True,
                    help="Name of the gene we are finding variants for.")
...
for file in vcf_file_list:
    ...
    for line in vcf:
        ...
        if chrom != gene_chrom:
            ...
            if int(position)< gene_start:
                continue
            elif int(position)> gene_stop:
                #print(position)
                break
...
with open(args.out, "w") as out:
    #Create Header
    out.write("#Variant Location Table for variants in gene "+ args.gene+ " .\n")
